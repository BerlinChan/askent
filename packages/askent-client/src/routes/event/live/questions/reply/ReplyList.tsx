...
const ReplyList: React.FC<Props> = ({
  ...
  const [replyLiveQueryInput, setReplyLiveQueryInput] =
  ...
  const [questionLiveQueryData, setQuestionLiveQueryData] =
  ...
  const hasNextPage = getHasNextPage(
    ...
    questionLiveQueryData?.replyCount
  );

  const { loading } = useReplyLiveQueryAudienceSubscription({
